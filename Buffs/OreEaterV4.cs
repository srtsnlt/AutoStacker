				...
				if (petProjectileNotSpawned && player.whoAmI == Main.myPlayer)
				{
					...
					for (int _type=Main.npc.Length-1; _type >= 0; _type--)
					{
						...
						if(npcTexture == "AutoStacker/NPCs/OreEaterV4")
						{
							...
							Projectile.NewProjectile(new EntitySource_SpawnNPC(), player.position.X + (float)(player.width / 2), player.position.Y + (float)(player.height / 2), 0f, 0f, ModContent.ProjectileType<Projectiles.OreEaterV4>() , 0, 0f, player.whoAmI, 0f, 0f);
