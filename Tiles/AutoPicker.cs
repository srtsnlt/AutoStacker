...
namespace AutoStacker.Tiles
{
    public class AutoPicker : ModTile
    {
        ...
        public override bool RightClick(int i, int j)
        {
            ...
            if (item.type == ModContent.ItemType<Items.AutoPickerController>())
            {
                ...
                if (autoPickerController.topLeft.X != -1 && autoPickerController.topLeft.Y != -1)
                {
                    ...
                    topLeftPicker = Common.AutoStackerCommon.GetOrigin(Player.tileTargetX, Player.tileTargetY);
        ...
        public override void HitWire(int i, int j)
        {
            ...
            int pickerChest = Common.AutoStackerCommon.FindChest(topLeftPicker.X, topLeftPicker.Y);
            if (pickerChest == -1)
            {
                return;
            }
            ...
            {
                Point16 Origin = Common.AutoStackerCommon.GetOrigin(picker.X, picker.Y);
                int fieldChest = Common.AutoStackerCommon.FindChest(Origin.X, Origin.Y);
        ...
        private void moveNext(int pickPower)
        {
            ...
            for (; ; )
            {
                ...
                Point16 Origin = Common.AutoStackerCommon.GetOrigin(x, y);
                int fieldChest = Common.AutoStackerCommon.FindChest(Origin.X, Origin.Y);
        ...
        {
            //chest
            int chestNo = Common.AutoStackerCommon.FindChest(topLeftRecever.X, topLeftRecever.Y);
            if (chestNo != -1)
            {
                //stack item
